{% macro deployments_should_run_as_non_root(framework, check_id) %}
  {{ return(adapter.dispatch('deployments_should_run_as_non_root')(framework, check_id)) }}
{% endmacro %}

{% macro default__deployments_should_run_as_non_root(framework, check_id) %}{% endmacro %}

{% macro bigquery__deployments_should_run_as_non_root(framework, check_id) %}
(
     select
        '{{ framework }}' as framework,
        '{{ check_id }}' as check_id,
        'Deployment containers should run as non-root' as title,
        ...
        CASE WHEN BOOL(JSON_QUERY(container, '$.securityContext.runAsNonRoot')) IS DISTINCT FROM true
            ...
            ELSE 'pass'
        END as status
    from {{ full_table_name("k8s_apps_deployments") }},
...
(
    select
        '{{ framework }}' as framework,
        '{{ check_id }}' as check_id,
        'Deployment initContainers should run as non-root' as title,
